...
--
-- Safe to run on an already-provisioned project; it only replaces two
-- functions and recreates one trigger. No data is touched.
-- Same content as supabase/migrations/20260828140000_harden_auto_playlists.sql
...
-- Expect: signup works. Verify by registering, then:
SELECT (SELECT count(*) FROM auth.users) users,
       (SELECT count(*) FROM public.profiles) profiles,
       (SELECT count(*) FROM public.playlists) playlists;
