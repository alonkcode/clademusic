...
-- PART 2/6: security-and-sections
...
CREATE INDEX IF NOT EXISTS idx_track_comments_parent_id ON public.track_comments(parent_id);
CREATE INDEX IF NOT EXISTS idx_nearby_activity_track_id ON public.nearby_activity(track_id);
CREATE INDEX IF NOT EXISTS idx_nearby_activity_artist ON public.nearby_activity(artist);
...
CREATE INDEX IF NOT EXISTS idx_play_history_track ON public.play_history(track_id);
...
CREATE OR REPLACE TRIGGER update_user_location_fuzzy
...
CREATE OR REPLACE TRIGGER update_user_2fa_secrets_updated_at
...
-- Performance indexes
create index if not exists idx_track_sections_track_id on public.track_sections(track_id);
create index if not exists idx_track_sections_label on public.track_sections(label);
...
-- ============================================================
-- bundle-fixes/00-compat-prelude.sql
